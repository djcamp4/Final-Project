...
**The workflow being improved:** Applicants submit resumes through the company website. Those resumes land in a queue where recruiters open and read them one by one, manually assessing each candidate against the job requirements. Once the pool has been reviewed, recruiters rank the candidates based on their notes and impressions, then begin reaching out to set up calls. Every step in this process is done by hand, in sequence, with no shared scoring standard between reviewers.
